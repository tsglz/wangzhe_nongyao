...
// 由玩家自己选择怎么移动
int player_move(Hero my_team[3])
{
  char move;

  printf("%s现在的剪刀:%d 石头:%d 布:%d\n", my_team[0].name,
         my_team[0].scissors, my_team[0].rock, my_team[0].paper);
  printf("%s现在的剪刀:%d 石头:%d 布:%d\n", my_team[1].name,
         my_team[1].scissors, my_team[1].rock, my_team[1].paper);
  printf("%s现在的剪刀:%d 石头:%d 布:%d\n", my_team[2].name,
         my_team[2].scissors, my_team[2].rock, my_team[2].paper);

  std::cout << "请选择你的英雄（0, 1, 2）：" << std::endl;
  ...
  random_move(&my_team[move]); // 随机选择移动的函数，返回所选择的移动方式
